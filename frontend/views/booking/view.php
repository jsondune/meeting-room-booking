                        ...
                        <div class="col-md-6">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    ...
                                    <div class="fw-bold"><?= Yii::$app->formatter->asDate($model->booking_date, 'php:l, d F Y') ?></div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                ...
                <div class="card-body">
                    <div class="timeline">
                        <div class="timeline-item completed">
                            ...
                            <div class="text-muted small"><?= Yii::$app->formatter->asDatetime($model->created_at, 'php:d M Y H:i') ?></div>
                        </div>
                        ...
                        <div class="timeline-item completed">
                            ...
                            <div class="text-muted small"><?= Yii::$app->formatter->asDatetime($model->updated_at, 'php:d M Y H:i') ?></div>
                    ...
                    <?php if ($model->status === 'approved'): ?>
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu w-100">
                                <li>
                                    <a class="dropdown-item" href="#" onclick="addToGoogleCalendar(); return false;">
                                        <i class="bi bi-google me-2 text-danger"></i>Google Calendar
                                    </a>
                                </li>
                                <li>
                                    <a class="dropdown-item" href="#" onclick="addToOutlookCalendar(); return false;">
                                        <i class="bi bi-microsoft me-2 text-primary"></i>Microsoft Outlook
                                    </a>
                                </li>
                                <li>
                                    <a class="dropdown-item" href="#" onclick="addToOffice365Calendar(); return false;">
                                        <i class="bi bi-cloud me-2 text-info"></i>Office 365 / Outlook.com
                                    </a>
                                </li>
                                <li><hr class="dropdown-divider"></li>
                                <li>
                                    <a class="dropdown-item" href="#" onclick="downloadICS(); return false;">
                                        <i class="bi bi-download me-2 text-success"></i>ดาวน์โหลด iCal (.ics)
                                    </a>
                                </li>
                            </ul>
                        </div>
...
// Calendar event data
$calendarTitle = addslashes($model->title ?? $model->meeting_title ?? '');
$calendarDesc = addslashes($model->description ?? $model->meeting_description ?? '');
$calendarLocation = addslashes(($model->room->name ?? '') . ' - ' . ($model->room->location ?? ''));
$bookingDate = date('Ymd', strtotime($model->booking_date));
$startTime = str_replace(':', '', substr($model->start_time, 0, 5)) . '00';
$endTime = str_replace(':', '', substr($model->end_time, 0, 5)) . '00';
$calendarStart = $bookingDate . 'T' . $startTime;
$calendarEnd = $bookingDate . 'T' . $endTime;

// For ICS file
$icsStart = date('Ymd\THis', strtotime($model->booking_date . ' ' . $model->start_time));
$icsEnd = date('Ymd\THis', strtotime($model->booking_date . ' ' . $model->end_time));
$icsUid = $model->booking_code . '@' . Yii::$app->request->hostName;

// For Outlook.com / Office 365 (ISO format)
$outlookStart = date('Y-m-d', strtotime($model->booking_date)) . 'T' . substr($model->start_time, 0, 5) . ':00';
$outlookEnd = date('Y-m-d', strtotime($model->booking_date)) . 'T' . substr($model->end_time, 0, 5) . ':00';
...
window.addToGoogleCalendar = function() {
    const url = 'https://calendar.google.com/calendar/render?action=TEMPLATE' +
        ...
        '&details=' + encodeURIComponent('{$calendarDesc}') +
        '&sf=true&output=xml';
    window.open(url, '_blank');
};

// Add to Microsoft Outlook (Desktop)
window.addToOutlookCalendar = function() {
    // Download ICS file which Outlook can open
    downloadICS();
};

// Add to Office 365 / Outlook.com (Web)
window.addToOffice365Calendar = function() {
    const url = 'https://outlook.office.com/calendar/0/deeplink/compose?' +
        'subject=' + encodeURIComponent('{$calendarTitle}') +
        '&startdt=' + encodeURIComponent('{$outlookStart}') +
        '&enddt=' + encodeURIComponent('{$outlookEnd}') +
        '&location=' + encodeURIComponent('{$calendarLocation}') +
        '&body=' + encodeURIComponent('{$calendarDesc}');
    window.open(url, '_blank');
};
...
window.downloadICS = function() {
    ...
    link.download = 'booking-{$model->booking_code}.ics';
