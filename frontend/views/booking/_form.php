                ...
                <div class="card-body">
                    ...
                    <div class="row" id="room-selection">
                        ...
                        <?php 
                            ...
                            $roomImageUrl = null;
                            if ($room instanceof \common\models\MeetingRoom) {
                                $primaryImage = $room->getPrimaryImage();
                                $roomImageUrl = $primaryImage ? $primaryImage->getUrl() : null;
                                $roomId = $room->id;
                                $roomBuildingId = $room->building_id;
                                $roomCapacity = $room->capacity;
                                $roomType = $room->room_type;
                                $roomNameTh = $room->name_th;
                                $roomBuildingName = $room->building ? $room->building->name_th : '-';
                                $roomFloor = $room->floor;
                                $roomHasProjector = $room->has_projector ?? false;
                                $roomHasVideoConference = $room->has_video_conference ?? false;
                            } else {
                                $roomImageUrl = !empty($room['images']) ? (is_array($room['images']) ? $room['images'][0] : null) : null;
                                $roomId = $room['id'];
                                $roomBuildingId = $room['building_id'] ?? '';
                                $roomCapacity = $room['capacity'] ?? 0;
                                $roomType = $room['room_type'] ?? '';
                                $roomNameTh = $room['name_th'];
                                $roomBuildingName = $room['building_name'] ?? '-';
                                $roomFloor = $room['floor'] ?? '-';
                                $roomHasProjector = $room['has_projector'] ?? false;
                                $roomHasVideoConference = $room['has_video_conference'] ?? false;
                            }
                        ...
                        <div class="col-md-6 col-lg-4 mb-3 room-item" 
                             data-building="<?= Html::encode($roomBuildingId) ?>"
                             data-capacity="<?= Html::encode($roomCapacity) ?>"
                             data-type="<?= Html::encode($roomType) ?>">
                            <div class="card room-card h-100 <?= $model->room_id == $roomId ? 'selected' : '' ?>" 
                                 data-room-id="<?= $roomId ?>">
                                <div class="position-relative">
                                    <?php if ($roomImageUrl): ?>
                                    <img src="<?= Html::encode($roomImageUrl) ?>" class="card-img-top" 
                                         alt="<?= Html::encode($roomNameTh) ?>" style="height: 120px; object-fit: cover;">
                                    <?php else: ?>
                                    ...
                                    <span class="position-absolute top-0 end-0 m-2 badge bg-primary">
                                        <i class="bi bi-people me-1"></i><?= $roomCapacity ?>
                                    </span>
                                </div>
                                <div class="card-body p-2">
                                    <h6 class="card-title mb-1"><?= Html::encode($roomNameTh) ?></h6>
                                    <p class="card-text small text-muted mb-1">
                                        <i class="bi bi-building me-1"></i><?= Html::encode($roomBuildingName) ?>
                                        <span class="mx-1">|</span>
                                        <i class="bi bi-geo-alt me-1"></i>ชั้น <?= Html::encode($roomFloor) ?>
                                    </p>
                                    <div class="d-flex flex-wrap gap-1">
                                        <?php if ($roomHasProjector): ?>
                                        <span class="badge bg-light text-dark" title="โปรเจคเตอร์"><i class="bi bi-projector"></i></span>
                                        <?php endif; ?>
                                        <?php if ($roomHasVideoConference): ?>
