...
<div class="row">
    ...
    <div class="col-lg-4">
        ...
        <div class="card mb-4">
            <div class="card-header">
                <i class="bi bi-images me-2"></i>รูปภาพห้องประชุม
            </div>
            <div class="card-body">
                <?php if (!$isNewRecord && $model->images): ?>
                    <div class="row g-2 mb-3" id="existingImages">
                        <?php foreach ($model->images as $image): ?>
                            <div class="col-6 position-relative" data-image-id="<?= $image->id ?>">
                                <img src="<?= $image->getUrl() ?>" alt="" class="img-fluid rounded" style="height: 80px; width: 100%; object-fit: cover;">
                                <?php if ($image->is_primary): ?>
                                    <span class="badge bg-primary position-absolute top-0 start-0 m-1">หลัก</span>
                                <?php endif; ?>
                                <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0 m-1 btn-remove-image" 
                                        data-image-id="<?= $image->id ?>" title="ลบรูปภาพ">
                                    <i class="bi bi-x"></i>
                                </button>
                            </div>
                        <?php endforeach; ?>
                ...
                <div class="mb-3">
                    <label class="form-label">เพิ่มรูปภาพ (สูงสุด 10 รูป)</label>
                    <input type="file" name="MeetingRoom[imageFiles][]" class="form-control" multiple accept="image/*" id="imageUpload">
                    <small class="text-muted">รองรับ JPG, PNG, GIF ขนาดไม่เกิน 5MB/รูป</small>
                </div>
                ...
                reader.onload = function(e) {
                    ...
                    col.innerHTML = `
                        <div class="position-relative">
                            ...
                            <span class="badge bg-secondary position-absolute bottom-0 start-0 m-1">ใหม่</span>
                        </div>
                    `;
